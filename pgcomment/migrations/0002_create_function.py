...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('pgcomment', '0001_initial'),
    ]

    operations = [
        migrations.RunSQL("""
            CREATE OR REPLACE FUNCTION pgcomment_add_comment (
                _id integer,
                _path integer[],
                _author text,
                _text text
        ...
        migrations.RunSQL("""
            CREATE OR REPLACE FUNCTION pgcomment_set_comment_attributes (
                _id integer,
                _path integer[],
                _new_value jsonb
            ) RETURNS VOID LANGUAGE SQL AS $$
            ...
            $$;
        """, reverse_sql="""
            DROP FUNCTION IF EXISTS pgcomment_set_comment_attributes (integer, integer[], jsonb);
        """),
    ]
